...
class HealthCheck(APIView):
    ...
    def get(request):
        ...
        # Optional Redis check - don't fail health check if Redis isn't ready
        try:
            health_check_key = 'health_check'
            cache.set(health_check_key, 'ok', 5)
            result = cache.get(health_check_key)

            if result == 'ok':
                response_data["redis"] = "connected"
            else:
                response_data["redis"] = "disconnected"
                logger.warning("Redis health check value mismatch")
        except Exception as e:
            response_data["redis"] = "disconnected"
            logger.warning(f"Redis health check failed: {str(e)}")

        # Always return 200 OK if the application is running
